...
class SWA(Optimizer):
    def __init__(self, optimizer, swa_start=None, swa_freq=None, swa_lr=None, max_lr=None, min_lr=None):
        ...
        self.cyclic = (self.swa_lr is None) and (max_lr is not None) and (min_lr is not None) and self._auto_mode and (not swa_freq < 2)
        if self.cyclic:
            self.base_lr = max_lr
            self.delta_lr = (max_lr-min_lr)/(swa_freq-1)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if self._auto_mode:
                if steps == self.swa_start and self.cyclic: group['lr']=self.base_lr
                if steps > self.swa_start and (steps-self.swa_start) % self.swa_freq == 0:
                    self.update_swa_group(group)
                    if self.cyclic: group['lr']=self.base_lr
                elif steps > self.swa_start: 
                    if self.cyclic: group['lr']=group['lr']-self.delta_lr

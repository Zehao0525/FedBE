...
from eval_utils.metrics_evaluator import metric_eval,metric_eval_flwr
# Note: 
...
if args.dataset == 'mnist':
    ...
    if args.iid:
        ...
        test_dict_users = cifar_test(dataset_test, args.num_users, num_data=len(dataset_test), cnts_dict = cnts_dict)
else:
...
def client_train(q, device_id, net_glob, iters, idx, val_id=server_id, generator=None, dset_train = None, d_users = None,dset_test = None,c_dict = None):
    ...
    local = SWAGLocalUpdate(args=args, 
                                ...
                                idxs=d_users[idx], 
                                server_ids=val_id, 
                                test=(dset_test, range(len(dset_test))), 
                                num_per_cls=c_dict[idx]   )
...
# loss, entropy : 
def server_train(q, device_id, net_glob, teachers, global_ep, w_org=None, base_teachers=None, serv_id = None, dset_eval = None,dset_test = None,tid = None):
        device=torch.device('cuda:{}'.format(device_id) if torch.cuda.is_available() and args.num_gpu != -1 else 'cpu')
        student = ServerUpdate(args=args, 
                            device=device, 
                            dataset=dset_eval, 
                            server_dataset=dset_eval, 
                            server_idxs=serv_id, 
                            train_idx=tid, 
                            test=(dset_test, range(len(dset_test))), 
                            w_org=w_org, 
...
def test_thread_uncert(q, net_glob, dataset, ids, thread_id = 0):
    ...
    ece_num_bins = 10
    b_size = 32
    device = torch.device('cuda:{}'.format(0) if torch.cuda.is_available() and args.num_gpu != -1 else 'cpu')

...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid:
            ...
            test_dict_users, test_cnts_dict = cifar_test(dataset_test, args.num_users, num_data=len(dataset_test), cnts_dict = cnts_dict)
    else:
    ...
    def test_thread_uncert(q, net_glob, dataset, ids, thread_id = 0):
        ensemble_num = 1
        ece_num_bins = 10
        b_size = 32
        device = torch.device('cuda:{}'.format(0) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
        dataloader = DataLoader(DatasetSplit(dataset, ids), batch_size=b_size, shuffle=False)
        met_dict = metric_eval_flwr(dataloader,net_glob,device,b_size,ensemble_num,ece_num_bins)
        met_dict.update({"tid":thread_id})
        q.put(met_dict)
        return met_dict


    def eval(net_glob, tag='', server_id=None, dset_eval = None,dset_test = None, tid = None):
        # testing
        q = mp.Manager().Queue()

        p_tr = mp.Process(target=test_thread, args=(q, net_glob, dset_eval, tid))  
        p_tr.start()
        p_tr.join()
        [acc_train, loss_train] = q.get()

        q2 = mp.Manager().Queue()
        p_te = mp.Process(target=test_thread, args=(q2, net_glob, dset_test, range(len(dset_test))))  
        p_te.start()
        p_te.join()
        ...
        q3 = mp.Manager().Queue()
        p_val = mp.Process(target=test_thread, args=(q3, net_glob, dset_eval, server_id))
        p_val.start()
        p_val.join()
        ...
        '''
        q4 = mp.Manager().Queue()
        p_uncert = mp.Process(target=test_thread_uncert, args=(q4, net_glob, dataset_test, range(len(dataset_test))))
        p_uncert.start()
        p_uncert.join()
        ...
        del q2 
        del q3 
        eval_dict = None #del q4

        return [acc_train, loss_train], [acc_test,  loss_test], [acc_val,  loss_val],eval_dict

    def met_eval(net_glob_list, tag='', eval_dict=None):
        # testing
        q = mp.Manager().Queue()

        p_tr = mp.Process(target=test_thread_uncert, args=(q, net_glob, dataset_eval, eval_dict))  
        p_tr.start()
        p_tr.join()
        eval_dict = q.get()
        del q
    ...
    def met_eval_list(net_glob_list, tag='', eval_id_dict=None):
        # testing
        q = mp.Manager().Queue()
        m = len(net_glob_list)
        num_threads = 5
        eval_res_list = [[] for i in range(args.num_users)]
        ...
        for i in range(0, m, num_threads):
            ...
            for idx in idxs_users[i:i+num_threads]:
                ...
                p = mp.Process(target=test_thread_uncert, args=(q, net_glob_list[idx], dataset_eval, eval_id_dict[idx], idx))  
                p.start()
                processes.append(p)
            ...
            while not q.empty(): 
                ...
                met_dict = q.get()
                idx = int(met_dict["tid"])
                eval_res_list[idx].append(met_dict)
        del q 

        eval_res_list = [c[0] for c in eval_res_list if len(c)>0]
        return eval_res_list


    def put_log(logger, net_glob, tag, iters=-1, serv_id=None, dset_eval=None,dset_test=None,tid=None):
        [acc_train, loss_train], [acc_test,  loss_test], [acc_val,  loss_val], eval_dict = eval(net_glob, tag=tag, server_id=serv_id,dset_eval=dset_eval,dset_test=dset_test,tid=tid)
    ...
    def put_uncert_log(logger, net_glob, ids, tag, iters=-1):
        eval_dict = met_eval(net_glob, tag = tag, eval_id = ids)
        if iters==0:
            open(os.path.join(args.uncert_dir, tag+"_acc.txt"), "w")
            open(os.path.join(args.uncert_dir, tag+"_b_score.txt"), "w")
            open(os.path.join(args.uncert_dir, tag+"_ece.txt"), "w")
            open(os.path.join(args.uncert_dir, tag+"_nll.txt"), "w")
            open(os.path.join(args.uncert_dir, tag+"_auroc.txt"), "w")
        with open(os.path.join(args.uncert_dir, tag+"_acc.txt"), "a") as f:
            f.write("%d %f\n"%(iters, eval_dict["correct"]/eval_dict["total"]))
        with open(os.path.join(args.uncert_dir, tag+"_b_score.txt"), "a") as f:
            f.write("%d %f\n"%(iters, eval_dict["b_score"]))
        with open(os.path.join(args.uncert_dir, tag+"_ece.txt"), "a") as f:
            f.write("%d %f\n"%(iters, eval_dict["ece"]))
        with open(os.path.join(args.uncert_dir, tag+"_auroc.txt"), "a") as f:
            f.write("%d %f\n"%(iters, eval_dict["nll"]))
        with open(os.path.join(args.uncert_dir, tag+"_acc.txt"), "a") as f:
            f.write("%d %f\n"%(iters, eval_dict["auroc"]))


    def put_uncert_log_clients(logger, net_glob_list, ids_list, tag, iters=-1):
        print("")
    ...
    for iters in range(args.rounds):
        ...
        for i in range(0, m, num_threads):
            ...
            for idx in idxs_users[i:i+num_threads]:
                # create process p and append to processes
                p = mp.Process(target=client_train, args=(q, idx%(args.num_gpu), copy.deepcopy(net_glob), iters, idx, server_id, generator,dataset_train, dict_users, dataset_test,cnts_dict))
                p.start()
                processes.append(p)
        ...
        if iters%args.log_ep== 0 or iters == args.rounds-1:
            put_log(
                ...
                iters=iters,
                serv_id =  server_id,
                dset_eval = dataset_eval,
                dset_test = dataset_test,
                tid = train_ids
            )
        ...
        p = mp.Process(target=server_train, args=(q, (args.num_gpu-1), net_glob, teachers_list, iters,None,None,server_id, dataset_eval, dataset_test, train_ids))
        p.start()
        p.join()
        ...
        if iters%args.log_ep== 0 or iters == args.rounds-1:
            ...
            put_log(dist_logger, net_glob, tag='DIST-SWA', iters=iters,serv_id =  server_id,
                dset_eval = dataset_eval,
                dset_test = dataset_test,
                tid = train_ids
            )
        ...
        if args.store_model_pipe and (iters%args.log_ep==0 or iters==args.rounds-1):
            ...
            if args.update=='FedAvg':
                ...
                store_model_2(iters,model_dir,w_glob_mean,tag='w_wSWA')  

        del clients
        print("redefined:",redefined)

    # Add Tagger
    net_glob.load_state_dict(w_glob_avg)
    torch.save(net_glob.state_dict(), os.path.join(args.log_dir, "FedBE_AVG_model"))
    net_glob.load_state_dict(w_glob_mean)
